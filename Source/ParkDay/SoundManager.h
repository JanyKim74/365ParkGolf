...
public:
    ...
    /**
     * ✅ 레벨 전환(OpenLevel) 직전에 호출: 재생 중인 BGM/Voice를 정지하고
     *    AudioComponent를 현재 월드에서 unregister 한다.
     *    월드가 파괴된 뒤 컴포넌트가 등록된 채 남아 발생하는
     *    "components incorrectly unregistered after world cleanup" ensure/크래시를 예방.
     *    (OnWorldCleanup 훅과 별개로, 전환 직전 명시적으로 호출해도 안전 — 멱등)
     */
    UFUNCTION(BlueprintCallable, Category = "Sound|Setup")
    void CleanupBeforeLevelTravel();
private:
    // Runtime audio components (transient; owned by this subsystem, registered to current world)
    UPROPERTY(Transient) UAudioComponent* BGMComp = nullptr;
    UPROPERTY(Transient) UAudioComponent* VoiceComp = nullptr;

    // ✅ 현재 컴포넌트가 등록된 월드 추적 (월드가 바뀌면 재등록)
    UPROPERTY(Transient) UWorld* RegisteredWorld = nullptr;
    ...
    // ✅ 월드 정리(레벨 전환/종료) 직전에 AudioComponent를 unregister →
    //    "components incorrectly unregistered after world cleanup" ensure/크래시 방지
    void HandleWorldCleanup(UWorld* World, bool bSessionEnded, bool bCleanupResources);

    // ✅ 컴포넌트를 현재 월드에서 안전하게 떼어내는 헬퍼
    void UnregisterAudioComponents();

    // OnWorldCleanup 델리게이트 핸들 (Deinitialize에서 해제)
