...
#ifdef _WIN32
    // [ParkDay 패치] Windows에서 Ort::Session은 ORTCHAR_T(wchar_t*) 경로를 요구.
    // 원본 예제는 char* 를 넘겨 Windows에서 컴파일 불가 → UTF-8 → UTF-16 변환.
    const std::wstring wide_path = std::filesystem::u8path(onnx_path).wstring();
    return std::make_unique<Ort::Session>(env, wide_path.c_str(), opts);
#else
